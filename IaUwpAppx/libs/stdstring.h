...
// Promote them to the global namespace so we can use them here.
	...
	//These code lines replaces ->(*)
	#if defined(_MSC_VER) && (_MSC_VER >= 1400) //if MSVC8 and later
		inline int	ssnprintf(PSTR pA, size_t sizeOfBuffer, size_t nCount, PCSTR pFmtA, va_list vl)
		{ 
			return _vsnprintf_s(pA, sizeOfBuffer, nCount, pFmtA, vl);
		}
		inline int	ssnprintf(PWSTR pW, size_t sizeOfBuffer, size_t nCount, PCWSTR pFmtW, va_list vl)
		{
			return _vsnwprintf_s(pW, sizeOfBuffer, nCount, pFmtW, vl);
		}
	#else
...
public:
	...
	bool TryLoad(const void* pT)
	{
		...
		if ( ( pT != NULL ) && SS_IS_INTRESOURCE(PtrToUlong(pT)))
		{
...
//		}
	...
	// With string buffer allocation. FT 25/08/2012
	void AppendFormat2(const CT* szFmt, ...)
	{
		va_list argList;
		va_start(argList, szFmt);
		AppendFormatV2(szFmt, argList);
		va_end(argList);
	}
	...
	void AppendFormatV(const CT* szFmt, va_list argList)
	{
		...
		#if defined(_MSC_VER) && (_MSC_VER >= 1400) //if MSVC8 and later
			int nLen = ssnprintf(szBuf, sizeof(szBuf), STD_BUF_SIZE-1, szFmt, argList);
		#else
	...
	// With string buffer allocation. FT 25/08/2012
	void AppendFormatV2(const CT* szFmt, va_list argList)
	{
		MYTYPE szBuf;

		#if defined(UNICODE)
			int nAppendLength = _vscwprintf(szFmt,argList);
		#else
			int nAppendLength = _vscprintf(szFmt, argList);
		#endif

		if (nAppendLength < 0)
			return;

		CT* pszBuffer = szBuf.GetBuffer(nAppendLength + 1);
		
		#if defined(_MSC_VER) && (_MSC_VER >= 1400) //if MSVC8 and later
			int nLen = ssnprintf(pszBuffer, nAppendLength + 1, nAppendLength, szFmt, argList);
		#else
			int nLen = ssnprintf(pszBuffer, nAppendLength, szFmt, argList);
		#endif
	...
	void MakeCapitalize()
	{
		...
		{
			MYTYPE strTemp;
			strTemp = (CT)(this->GetAt(0));
			this->ToLower();
			strTemp.ToUpper();
			this->SetAt(0, strTemp.GetAt(0));
		}
	}
